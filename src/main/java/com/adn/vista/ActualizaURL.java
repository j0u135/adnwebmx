 ...
 */
@WebServlet("/actualizaURL")
public class ActualizaURL extends HttpServlet {
			   ...
			   if(arrActividad!=null && arrActividad.size()>0){
				...
				if(contComent!=null && !contComent.equals("") && !contComent.equals("0")){
					...
					for(int j=1; j<=cantURL; j++){
						...
						String nbURL = request.getParameter("nombreURL"+j);
						String desURL = request.getParameter("descURL"+j);
						urlDet.setIdPrioridad(prio.getIdPrioridad());
