...
var mesesGraficas=["May14","Jun14", "Jul14","Ago14","Sep14","Oct14", "Nov14","Dic14","Ene15","Feb15","Mar15", "Abr15",];
...
var ventasMto=[3.7,3.8,4.3,5.5,5,4.5,4.4,4.4,3.30,2.64,1.32,0.33];
var ventasProductos=[3.8,3.5,3.2,3,3.6,3.3,3.4,3.6,3.5,3.3,3.9,3.8,3.7];
var ventasConsultoria=[3.6,3.5,3,3.23,3.76,3.89,3.7,3.3,3.98,3.93,3.74,3.96];
var satisfaccion=[100,100,86,90,89,93,96,90,100,80,40,10];
...
var ingresosFlujo=[12,11.2,12,10.8,11,11.5,11.4,10,10.57,10.53,8.88,8.09];
...
function datosGraficar(){
	//alert("cambio frecuencia "+cambioFrecuencia+" variable frecuencia "+variableFrecuencia);
	//alert(datosIndicadores+ " entrando a graficar");
...
function graficasInicio5(){

	document.getElementById("contenedorGraficas").innerHTML="";	
	
	nombreIndicador="Satisfacción del cliente";
	tipoGrafica="satisfaccion1";
	crearHerramienta();
}
function graficasInicio4(){

	 document.getElementById("contenedorGraficas").innerHTML="";	
	
	nombreIndicador="Ventas Totales";
	tipoGrafica="ventas1";
	crearHerramienta();
	nombreIndicador="Ventas Mantenimiento";
	tipoGrafica="ventasM1";
	crearHerramienta();
	nombreIndicador="Ventas Productos";
	tipoGrafica="ventasP1";
	crearHerramienta();
	nombreIndicador="Ventas Consultoria";
	tipoGrafica="ventasC1";
	crearHerramienta();
...
function ventasC(){
	document.getElementById("grafica"+indice).style.heigth="250px";
	document.getElementById("grafica"+indice).style.width="300px";
	//alert("entra");
	 $("#grafica"+indice).highcharts({
		 title: {
	            text: nombreIndicador,
...
// 	 destino.appendChild(nuevo);
	 ...
	 //Crea Labels
// 	 original=document.getElementById("labelIndicador");
// 	 nuevo=original.cloneNode(true);
// 	 nuevo.id="labelIndicador"+indice;
// 	 destino=document.getElementById("tdIndicador"+indice);
// 	 destino.appendChild(nuevo);
...
// 	 var imagen= document.createElement("img"); //creas un elemento de imagen.
// 		if(contadorMetas>2){
// 			imagen.setAttribute("src", "ADNFiles/alertaRoja.png");
// 		}else if(contadorMetas<=2){
// 			imagen.setAttribute("src", "ADNFiles/alertaAmarilla.png");
// 		}else{
// 			imagen.setAttribute("src", "ADNFiles/alertaVerde.png");
// 		}
		
	    //ahora le acabas de agregar el src de la imagen (la ruta de donde se encuetra). 

// 		var bloq=document.getElementById("tdIndicador"+indice); //ahora tomas la referencia al elemento de bloque. 

// 		bloq.appendChild(imagen); // y despues insertas la imagen en el elemento de bloque (div). 
	 //Asigna valores Label
	// document.getElementById("labelIndicador"+indice).innerHTML=nombreIndicador;
...
function llamado(){
	...
	if(gGraficaDD=="tabla"){
		...
		//Se deshabilitan las alertas por ahora 27 Ene 2015
		//agregaAlertas();
		historiaDashboard(gGraficaDD);
	}
...
// 		objetoSeleccion=document.getElementById("herramienta"+(indice-1));
// 		alert(objetoSeleccion);
// 		objetoSelPos=objetoSeleccion.position();
// 		alert(objetoSelPos);
...
function splitFuncion(){
	...
	dashboardunido = document.getElementById("ultimoDasActivo").value;
	
	//alert(dashboardunido);
		// prueba de como obtiene el string para armar graficas "herramienta1,Almacen,350px,gLineal,posicion-,herramienta2,Ventas,350px,gLineal,posicion-,herramienta3,Empleado,350px,gDona,posicion-,herramienta4,Almacen,350px,gBarrasConNegativo,posicion-,herramienta5,Ventas,350px,gLineaCombinada,posicion-";
	if(dashboardunido!=""){
		...
		for(i=0;i<numeropalabras;i++){
			...
			//Posiciones
			//1-nombre div 2.-nombre indicador 3.-tamaño 4.-tipo de grafica 5.- Posicion
...
<div id="cabecera">
   ...
   <br>
    <table align="center" width="1200px" style="display:none">
        <tr>
    ...
    <table align="center" width="1200px" >
    	<tr>
    		<td>
    			...
    			<input type="button" value="Comercial" onclick="graficasInicio4()">
    			<input type="button" value="Operaciones" onclick="graficasInicio5()">
    			<input type="button" value="Utilidad" onclick="graficasInicio2()">
    			<input type="button" value="Resultado" onclick="graficasInicio()">
    			<input type="button" value="Flujo" onclick="graficasInicio3()">
    			<input type="button" value="Contribución" onclick="linkDir('Italika2.jsp')">
        ...
        <tr>
        	<%
				...
				busquedaDas="SELECT contenido FROM dashboard where activo='true'";
			...
			%>
				 <input type="hidden" value="<%=stringDas%>" name="ultimoDasActivo" id="ultimoDasActivo">
            ...
            <td width="95%" valign="top">
            	<form id="guardaDas" action="guardarDashboard" method="post">
	                <div width="100%" height="800px" id="contenedorGraficas" name="contenedorGraficas"></div>
            ...
            <td width="5%" align="center" valign="top">
                    ...
                    <table width="80%" BGCOLOR="#f5f6f6" border="0">
                    	<tr>
                    		<td align="center">
                        ...
                        <tr>
                        	<td align="center">
			               	<%
								String nombre="", Q1="", busquedaq="", busquedamen="", Q2="", semana="", busquedasem="", tri1="",tri2="",tri3="",tri4="", Q3="", busquedaf="", Q4="", busqueda="",ene="",feb="",mar="",abr="",may="",jun="",jul="",ago="",sep="",oct="",nov="",dic="",frecuencia="";
							%>
			                   <select id="selectIndicador" class="selectEstilo" onchange="seleccionFrecuencia(this.value);asignaMeta(this.value);muestraGraficas();datosGraficar()">
			                       <option value="noSeleccionado">Indicador</option>
			
			                   <%
			                   	Operaciones obj0=new Operaciones();
								busquedaf="SELECT Nombre, Frecuencia FROM indicadores";
								ResultSet rs1=obj0.consultar(busquedaf);
																	
									while(rs1.next()){
									nombre=rs1.getString(1);
									frecuencia=rs1.getString(2);
										if(frecuencia.equals("semanal"))
											{
											Operaciones obj1=new Operaciones();
											busquedasem="SELECT semana FROM indicadores where nombre='"+nombre+"'";
											ResultSet rs2=obj1.consultar(busquedasem);
												while(rs2.next())
												{
													semana=rs2.getString(1);
													%>
											 <option value="<%=nombre+"-"+semana+"-"+frecuencia%>" ><%=nombre%></option>
													<%
												}
											}
										else if(frecuencia.equals("mensual"))
												{
													...
													busquedamen="SELECT ene,feb,mar,abr,may,jun,jul,ago,sep,oct,nov,dic FROM indicadores where nombre='"+nombre+"'";
													ResultSet rs3=obj2.consultar(busquedamen);
													while(rs3.next())
													{
														ene=rs3.getString(1);
														feb=rs3.getString(2);
														...
														ago=rs3.getString(8);
														sep=rs3.getString(9);
														oct=rs3.getString(10);
														nov=rs3.getString(11);
														dic=rs3.getString(12);
														
														%>
														 <option value="<%=nombre+"-"+ene+"-"+feb+"-"+mar+"-"+abr+"-"+may+"-"+jun+"-"+jul+"-"+ago+"-"+sep+"-"+oct+"-"+nov+"-"+dic+"-"+frecuencia%>" ><%=nombre%></option>
														<%
													}
												}	
												else
												{
													...
													busquedaq="SELECT q1,q2,q3,q4 FROM indicadores where nombre='"+nombre+"'";
													ResultSet rs4=obj3.consultar(busquedaq);
													
														while(rs4.next()){
														tri1=rs4.getString(1);
														tri2=rs4.getString(2);
														tri3=rs4.getString(3);
														tri4=rs4.getString(4);
														//resultado2=rs.getString(2);
													
												%>
													 <option value="<%=nombre+"-"+tri1+"-"+tri2+"-"+tri3+"-"+tri4+"-"+frecuencia%>" ><%=nombre%></option>
												<%
												}
											}
										//resultado2=rs.getString(2);
...
<div id="herramientas"  class="" style="display:none">
